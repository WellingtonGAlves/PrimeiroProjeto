	  ...
	  # If we know the imei of the phone and there is a pending command send it.
	    if ($conn_imei != "")
		{
			...
			else 
			{
				...
				{
					//Se nao tiver comando na fila e for a primeira iteracao, obtem o ultimo comando vÃ¡lido enviado
							...
							if ( $result <> $resultCerca AND round($speed * 1.852,0) > 0 ) {
								...
								if ($tipoEnvio == 0) {
									...
									while($data = mysql_fetch_assoc($consulta1)) {
    									...
    									$mail->SMTPAuth = true;
    									// Indicando que o e-mail está no formato HTML
										$mail->IsHTML(true);
										// Indicando que o e-mail está na codificação UTF8
										$mail->CharSet = "utf-8";
										// Indicando tipo de segurança
										$mail->SMTPSecure = "ssl";
