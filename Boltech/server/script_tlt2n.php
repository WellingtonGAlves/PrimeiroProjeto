...
function interact($socket) {
    ...
    $conexao = mysql_connect("localhost", "user1", "pass1");
            ...
            if (preg_match("/GPRMC/i", $rec)) {
                ...
                for ($i = 1; $i <= $quantidadecoordenadas; $i++) {
                    ...
                    if ((count($parts) > 1) and ($gpsSignalIndicator == 'A')) {
						...
						mysql_query("INSERT INTO gprmc (date, imei, satelliteFixStatus, latitudeDecimalDegrees, latitudeHemisphere, longitudeDecimalDegrees, longitudeHemisphere, speed, infotext, gpsSignalIndicator) VALUES (now(), '$imei', 'A', '$latitude', '$latitudeHemisphere', '$longitude', '$longitudeHemisphere', '$speed', '$status', '".$gpsSignalIndicator."')", $conexao);
						...
						if($numRows == 0)
						{
							mysql_query("
								...
								values (now(), '$imei', '$latitudeDecimalDegrees', '$latitudeHemisphere', '$longitudeDecimalDegrees',
								        '$longitudeHemisphere', '$speed', '$gpsSignalIndicator', 1)", $conexao);
						} 
						else 
						{
							mysql_query("
								update loc_atual set date = now(), latitudeDecimalDegrees = '$latitudeDecimalDegrees', 
