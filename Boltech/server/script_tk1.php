								...
								while($data = mysql_fetch_assoc($consulta)) {
									...
									if ( $result <> $resultCerca And round($speed * 1.852,0) > 0 ) {
										...
										if ( $tipoEnvio == 0 ) {
											...
											$tempstr = "http://maps.google.com/maps/geo?q=$lat_point,$lng_point&oe=utf-8&sensor=false&key=AIzaSyAHtRwtz8ReaYTsyPOD83ojXSgcGw-koe0"; //output = csv, xml, kml, json
...
function enviaEmail($destinatario, $assunto, $msgConteudo){
	...
	$mail->AddReplyTo($mail->From, $mail->FromName);
	$mail->Subject 		= "TELOCALIZEI - Alerta ".$assunto;
	$mail->Body 		= $msgConteudo;

	if (!$mail->Send()) {
		printLog($fh, $mail->ErrorInfo);
		echo "Erro de envio: ". $mail->ErrorInfo;
	} else {
		printLog($fh, 'enviou');
		echo "Mensagem enviada com sucesso!";
