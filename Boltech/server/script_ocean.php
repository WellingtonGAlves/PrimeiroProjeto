...
/*
$conIni = mysql_connect("localhost", "user1", "pass1") 
						  or die("Could not connect: " . mysql_error());
mysql_select_db('tracker', $conIni);

$res = mysql_query("select valor from preferencias where nome = 'host_apn'", $conIni);
$dataIp = mysql_fetch_assoc($res);
$res = mysql_query("select valor from preferencias where nome = 'port_apn'", $conIni);
$dataPorta = mysql_fetch_assoc($res);
$res = mysql_query("select valor from preferencias where nome = 'email_alertas'", $conIni);
...
function interact($socket) {
	...
	while (@socket_recv($socket, $rec, 2048, 0x40) !== 0) {
	  ...
	  # If we know the imei of the phone and there is a pending command send it.
		...
		//try{
		if ($rec != "") {
			//mysql_query("INSERT INTO message (imei, message) values('$imei', '$rec')", $cnx);
			if (strpos($rec, "GPRMC") === false) {
				...
				if (strpos($parts[0], "#") === FALSE) {
					...
					*/
					...
					$resCliente = mysql_query("select id, celular, dt_ultm_sms, envia_sms, sms_acada, hour(timediff(now(), IFNULL(dt_ultm_sms,now()))) horas, minute(timediff(now(), IFNULL(dt_ultm_sms,now()))) minutos, nome from cliente where id = $dataBem[cliente]", $cnx);
						...
						try{
							...
							if($dataCliente['envia_sms'] == 'S' && $dataBem['envia_sms'] == 'S' && !empty($dataCliente['celular']) && !empty($dataCliente['sms_acada'])){
								if(empty($dataCliente['dt_ultm_sms'])){
									mysql_query("update cliente set dt_ultm_sms = now() where id = $dataCliente[id]",$cnx);
								} else {
				...
				*/
				if (count($parts) > 1) {
					...
					if($gpsSignalIndicator != 'L') {
						...
						$json = json_decode(file_get_contents("http://maps.googleapis.com/maps/api/distancematrix/json?sensor=false&origins=$gpsLatAnt,$gpsLonAnt&destinations=$latitudeDecimalDegrees,$longitudeDecimalDegrees"));
						
						mysql_query("insert into controle(texto) values('$json')", $cnx);
...
function sendSMS($contato, $mensagem, $remetente){
	/*
	$cnxSms = mysql_connect("localhost", "user1", "pass1") 
						  or die("Could not connect: " . mysql_error());
						mysql_select_db('tracker', $cnxSms);
	$res = mysql_query("select valor from preferencias where nome = 'url_sms'", $cnxSms);
	$data = mysql_fetch_assoc($res);
	$url = $data['valor'];
	
	$res = mysql_query("select valor from preferencias where nome = 'usuario_sms'", $cnxSms);
	$data = mysql_fetch_assoc($res);
	$usuario = $data['valor'];
	
	$res = mysql_query("select valor from preferencias where nome = 'senha_sms'", $cnxSms);
	$data = mysql_fetch_assoc($res);
	$senha = $data['valor'];
	
	$res = mysql_query("select valor from preferencias where nome = 'de_sms'", $cnxSms);
	$data = mysql_fetch_assoc($res);
	$de = $data['valor'];
