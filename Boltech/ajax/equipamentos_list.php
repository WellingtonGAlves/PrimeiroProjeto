...
$consulta = "SELECT b.*, la.longitudeDecimalDegrees, la.longitudeHemisphere, la.latitudeDecimalDegrees,
			...
			$sqlCliente $query LIMIT $start,20";
...
<div class="row">
	<div class="col-lg-8">
		<form class="form-inline" role="form" onsubmit="javascript:return(false);">
			...
			<a href="Boltech/javascript:pesquisa();" class="btn btn-primary">Pesquisar</a>
		</form>
	</div>
	<div class="col-lg-4">
		<ul class="pager">
			<li><a href="Boltech/javascript:paginacao(<?=$pagina>1?$pagina-1:$pagina?>)">&larr; Anterior</a></li>
			<li><a href="Boltech/javascript:paginacao(<?=$pagina<$count?$pagina+1:$pagina?>)">Próximo &rarr;</a></li>
		</ul>
	</div>
</div>
	...
	<?php
		...
		while($data = mysql_fetch_assoc($resEquip)):?>
			<tr>
				...
				?>
				<td class="text-center"><input type="checkbox" id="mapa<?=$data['imei']?>" onchange="javascript:carregarMapa('<?=$data['imei']?>');" value="<?=$data['imei']?>"/></td>
			    <td style="display:none"><?=$data['imei']?></td>
			    <td id="name_<?=$data['imei']?>"><?php echo "<a href='javascript:void(0);' onclick=\"javascript:showConfirm('show'," . $data['imei'] . ");\">" . $data['name'] . "</a>"; ?></td>
			    <td id="apelido_<?=$data['imei']?>"><?=$data['apelido'] . " / <b>" . $data['identificacao'] . "</b>" ?></td>
			    <td style="display:none" id="identificacao_<?=$data['imei']?>"><?=$data['identificacao']?></td>
			    <td style="display:none" id="apelido_<?=$data['imei']?>"><?=$data['apelido']?></td>
			    <td id="modelo_<?=$data['imei']?>"><?=$data['modelo_rastreador']?></td>
			    <td><?php
					...
					if ($data['ligado'] == 'S') {echo "<img src='imagens/chave1.png' alt='Ligado' title='Veiculo Ligado'> ";
                                ...
                                if ($data['bloqueado'] == 'N'){
                                    echo " <img src='imagens/unlock.png' alt='Veículo Desbloqueado' title='Veículo Desbloqueado'>";
                                    }else{ 
                                        echo " <img src='imagens/locked.png' alt='Bloqueado' title='Veículo Bloqueado'>";
                                        echo " <b>".$data['speed']." Km/h</b>";
                                    }
			    	?></td>
			    <td style="display:none"><?=$data['movimento']?></td>
			    <td style="display:none"><?=$data['speed']?></td>
			    <td style="display:none" id="endereco_<?=$data['imei']?>"><?=$address?></td>
			    <td style="display:none" id="veiculoLatitude<?=$data['imei']?>"><?=$latitudeDecimalDegrees?></td>
			    <td style="display:none" id="veiculoLongitude<?=$data['imei']?>"><?=$longitudeDecimalDegrees?></td>
			    <td><?=$data['ladata']?></td>
			    <td><?=$data['lahora']?></td>
			    <td><?php echo "<img src='../imagens/". imagenStatusSinal($data['status_sinal'], $atraso). "'>" ?></td>
			    <td style="display:none" id="tipo_<?=$data['imei']?>"><?=$data['tipo']?></td>
                            <td style="display:none" id="sinal_<?=$data['imei']?>"><?php if ($atraso){ echo "D"; }else{ echo $data['status_sinal'];} ?></td>
			    <td style="display:none" id="block_<?=$data['imei']?>"><?=$data['bloqueado']?></td>
			    <td>
			    <?php
      ...
      <form method="POST" action="listagem_historico_novo.php" class="form-inline" role="form" id="consultarHistorico">
        ...
        <div class="row"> 
          <div class="col-lg-9"> 
            ...
            <div class="form-group">
		<select name="cmbLimite" id="cmbLimite" class="form-control">
                ...
                <option value="2000">2000 Recentes</option>
              </select> 
