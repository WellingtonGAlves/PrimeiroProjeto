...
-->
<link href="Boltech/css/jquery.contextMenu.css" rel="stylesheet" type="text/css" />
...
<script type="text/javascript" src="javascript/jquery.ui.position.js"></script>
...
<script type="text/javascript"> 
	...
	function updateMyKml(imei) {
		if (imei != '') {
			...
			if (imei == 'ALL' || imei.indexOf('grupo') > -1) {
				...
				myParser.parse("Boltech/server/all_kml.php?cliente="+<?=$cliente?>+"&grupo="+grupo);
				...
				myParser.parse("Boltech/server/kml.php?entries=20&imei=" + imei);
	...
	function updateMyKmlHistorico(imei, dataInicial, dataFinal) {
		if (imei != '') {
			...
			if (imei == 'ALL' || imei.indexOf('grupo') > -1) {
				...
				myParser.parse("Boltech/server/kml.php?entries=20&imei=" + imei+'&data_inicial='+dataInicial+'&data_final='+dataFinal);
	...
	function addMarker(lat, lon, obj){
		...
		switch (obj.tipo) {			
			case 'MOTO':
				var image = 'Boltech/imagens/marker_moto' + imgTipo;
			break;
			
			case 'CARRO':
				var image = 'Boltech/imagens/marker_carro' + imgTipo;
			break;
			
			case 'JET':
				var image = 'Boltech/imagens/marker_jet' + imgTipo;
			break;
			
			case 'CAMINHAO':
				var image = 'Boltech/imagens/marker_caminhao' + imgTipo;
			break;
			
			case 'VAN':
				var image = 'Boltech/imagens/marker_van' + imgTipo;
			break;
			
			case 'PICKUP':
				var image = 'Boltech/imagens/marker_pickup' + imgTipo;
			break;
			
			case 'ONIBUS':
				var image = 'Boltech/imagens/marker_onibus' + imgTipo;
			break;
	...
	function addRota() {
		...
		if (true) {
			...
			var image = new google.maps.MarkerImage('Boltech/imagens/marcador_mapa.gif', new google.maps.Size(32, 32), new google.maps.Point(0,0));
		...
		if (contTotal == points.length) {
			...
			parent.bottom.document.getElementById('playRotaHistorico').src='Boltech/imagens/play_rota_historico.jpg';
		}
	}
...
input.smallCheckbox {
	...
	<body onload="resizeDiv();initialize();updateMyKml(document.getElementById('nrImeiMapa').value);" onresize="resizeDiv();" onunload="">
		...
		Visualize o <a style="color:#FFFFFF" href="/Boltech//map_all_devices.php" target="_blank">Mapa Grande</a>, Com todos os veiculos.
	</div>
		...
		<div style="height:5px;">
			<span class="linkStyle">
				...
				<a href="Boltech/javascript:;" onclick="abreFecha('pano');" class="linkStyle">exibir street view</a> 
