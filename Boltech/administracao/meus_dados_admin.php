...
if ($cliente != "master") {
	header("Location: /Boltech/logout.php");
...
if ($apelido != null and $novaSenha != null and $senhaAtual != null) {

	/*if ($apelido != null and strpos($apelido, "") === false) 
	{
		$sucesso = "<span id=\"alertaCadastro\" style=\"color:red\">Não digite espaços em branco no login.</span>";
		//$acao = "novoUsuario";
		$erro = true;
	}*/
	...
	if (!$erro) {
		...
		$sql = "UPDATE cliente set senha = '". md5($novaSenha) ."' WHERE apelido = '$apelido' and master = 'S' and senha = '". md5($senhaAtual) ."'";
...
$resUsuario = mysql_query("select apelido from cliente where master = 'S' limit 1");
...
<script type="text/javascript" src="Boltech/javascript/alterarVeiculo.js"></script>
		...
		<td class="tb-conteudo" colspan="4">
			<div id="div_cadastro" class="conteudo" style="display:block;">
				<div>
					...
					<form name="alteraDados" method="post" action="meus_dados_admin.php" autocomplete="off">
						<table style="width: 70%" cellspacing="6" cellpadding="0">
							...
							<tr>
								...
								<td><input name="apelido" maxlength="30" size="20" type="text" class="campoNovoVeiculo" value="<?php echo $apelido ?>" readonly />
							...
							<tr>
								<td colspan="2">
									...
									&nbsp;
									<a href="admin.php" style="color:#0099FF">Voltar</a>
								</td>
							</tr>
						</table>
					</form>
