...
$countCliente = 0;
$res = mysql_query("select count(*) as countCliente from cliente where master = 'N' and id_admin = $id_admin");
...
$countIMEI = 0;
$res = mysql_query("select count(*) as countIMEI from bem where id_admin = $id_admin");
...
if ($acao == "atualizarUsuario") {
	if ($email != null and $nomeCliente != null and $codigoCliente != null) {
		...
		{
			if ($senha != null) 
				$sql = "UPDATE cliente set nome = '$nomeCliente', email = '$email', senha = '". md5($senha) ."' WHERE id = $codigoCliente and master = 'N' and id_admin = $id_admin";
			else
				$sql = "UPDATE cliente set nome = '$nomeCliente', email = '$email' WHERE id = $codigoCliente and master = 'N' and id_admin = $id_admin";		
			...
			else
			{
				...
				$resCodigo = mysql_query("select id from cliente where email = '$email' and master = 'N' and id_admin = $id_admin");
...
if ($acao == "novoUsuario") {
	if ($email != null and $nomeCliente != null) {
		...
		{
			if (!mysql_query("INSERT INTO cliente (email, nome, senha, id_admin) VALUES ('$email', '$nomeCliente', '". md5($senha) ."', $id_admin)", $cnx))
			{
				// Se der erro, envia alerta que houve falha
				if (mysql_error() == "Duplicate entry '". $email ."' for key 'email_unq'") {
					$sucesso = "<span id=\"alertaCadastro\" style=\"color:red\">Usuário já existe!</span>";
...
if ($acao == "obterUsuario") {
	if ($codigoCliente != null) {
		...
		$resUsuario = mysql_query("select * from cliente where id = '$codigoCliente' and master = 'N' and id_admin = $id_admin");
...
<table width="80%" height="80%" cellspacing="0" cellpadding="0" border="0" style="border-left: 1px solid #000000; border-left-color: #CCCCCC;" align="center">
	...
	<tr>
		...
		onclick="AlternarAbas('td_termos','div_termos')" style="height: 7px;display:none">
			Termos
		</td>		
	...
	<tr>
		<td class="tb-conteudo" colspan="7">
			<div id="div_cadastro" class="conteudo" style="display:block;">
				<div>
					Adicione um usuário <br />

					<form name="novoUsuario" id="novoUsuario" method="post" action="admin_cliente.php" autocomplete="off">
						...
						<table style="width: 70%" cellspacing="6" cellpadding="0">
							...
							<tr>
								...
								<a href="admin_cliente.php" style="color:#0099FF">Novo</a>
								</td>
								<td></td>
							</tr>
						</table>
						...
						<td class="tb-sub-conteudo" colspan="4">
							<div id="div_sub_cadastro" class="conteudo" style="display:block; border-right: 1px solid #000000; border-right-color: #CCCCCC;">
								...
								<table cellspacing="6" cellpadding="0" id="tabelaVeiculos">
									...
									$res = mysql_query("select * from bem where cliente = $codigoCliente and id_admin = $id_admin order by name");
				...
				<div>
					...
					<table cellspacing="6" cellpadding="0">
						...
						$resUsu = mysql_query("select CAST(c.id AS DECIMAL(10,0)) as id, c.id as codCliente, c.email, c.nome, c.ativo, 
											  ...
											  from cliente c 
											  where c.master = 'N' and c.id_admin = $id_admin
											  order by nome");
						...
						for ($i=0; $i < mysql_num_rows($resUsu); $i++) {
							...
							echo "<tr>";
								...
								echo "<td><select id='listaAtivoCliente". $rowUsu[id] ."' name='listaAtivoCliente". $rowUsu[id] ."' class='campoNovoVeiculo'>";
									...
									echo "</select>";
								echo "</td>";								
								echo "<td valign='top' style='color:black;font-weight:bold'> <div style='width:46px'> <a href='javascript:void(0);'> <img border=0 src='../imagens/frota.gif' style='height:25px' title='Frota do cliente' alt='Frota do cliente' onclick='abrirFrotaClienteSub(". $rowUsu[id] .");' /> </a> <sup>". $rowUsu[qtFrota] ."</sup></div></td>";
				...
				Listagem de pagamentos <br />
					...
					<table cellspacing="6" cellpadding="0">
						...
						$resPag = mysql_query("select CAST(c.id AS DECIMAL(10,0)) as id,
													  ...
													  where c.ativo = 'S' and c.master = 'N' and c.id_admin = $id_admin and
			...
			<div id="div_status" class="conteudo" style="display: none">
				...
				<span style="font-size:10px">
					...
					<a href='admin_cliente.php?aba=status' style='color:#0099FF'>atualizar agora</a>
				</span> 
				<br /><br />
				...
				<table cellspacing="6" cellpadding="0">
						...
						$resAcesso = mysql_query("select b.name, b.identificacao, b.imei, b.status_sinal, c.id, c.nome 
												  from bem b inner join cliente c on (c.id = b.cliente) 
												  where c.ativo = 'S' and c.master = 'N' and c.id_admin = $id_admin 
												  order by c.nome");
						...
						for ($k=1; $k <= mysql_num_rows($resAcesso); $k++) 
						{
							...
							echo "<tr id='stKey". $k ."'>";
								...
								echo "<td><a href='admin_cliente.php?acao=obterUsuario&codigo=". $rowAcesso[id] ."' style='color:#0099FF'>". $rowAcesso[nome] ."</a></td>";
			...
			<div id="div_hist_com" class="conteudo" style="display: none">
				...
				<form name="listaComandos" method="post" action="admin_cliente.php">
				...
				<table cellspacing="6" cellpadding="0">
						...
						$resHistCom = mysql_query("select b.name, b.identificacao, b.imei, b.status_sinal, c.id, c.nome, l.data, l.ip
												   ...
												   where c.ativo = 'S' and c.master = 'N' and id_admin = $id_admin and
														 l.data between '$dtIniBloqueioSql 00:00:00' and '$dtFimBloqueioSql 23:59:59'
						...
						for ($k=1; $k <= mysql_num_rows($resHistCom); $k++) 
						{
							...
							echo "<tr>";
								...
								echo "<td><a href='admin_cliente.php?acao=obterUsuario&codigo=". $rowHistCom[id] ."' style='color:#0099FF'>". $rowHistCom[nome] ."</a></td>";
