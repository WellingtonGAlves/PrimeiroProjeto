...
<script type="text/javascript"> 
	...
	function updateMyKml(imei) {
		if (imei != '') {
			...
			limparMapa();
			if (imei == 'ALL')
				kmlLayer = new google.maps.KmlLayer("http://<?php echo $_SERVER['SERVER_NAME'] ?>/Boltech/server/all_kml.php?entries=<?php echo $points;?>&imei=" + imei + "&date="+ (new Date()).getDate(), kmlLayerOptions);
			else
				kmlLayer = new google.maps.KmlLayer("http://<?php echo $_SERVER['SERVER_NAME'] ?>/Boltech/server/kml.php?entries=<?php echo $points;?>&imei=" + imei + "&date="+ (new Date()).getDate(), kmlLayerOptions);
	...
	function addRota() {
		...
		if (true) {
			...
			var image = new google.maps.MarkerImage('Boltech/imagens/marcador_mapa.gif',
