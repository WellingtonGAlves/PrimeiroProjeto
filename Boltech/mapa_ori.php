...
<script type="text/javascript" src="Boltech/javascript/geoxml3.js"></script>
<script type="text/javascript" src="Boltech/javascript/ProjectedOverlay.js"></script>
<script type="text/javascript" src="Boltech/javascript/ge.js"></script>
...
<script type="text/javascript"> 
	...
	function updateMyKml(imei) {
		...
		if (imei != '') 
		{
			...
			if (imei == 'ALL') {
				//kmlLayer = new google.maps.KmlLayer("http://localhost/sistema/server/all_kml.php?entries=20&imei=" + imei, kmlLayerOptions);
				//myParser.parse("server/all_kml.php?entries=20&cliente=672");
				
				if (chk.checked == true)
					myParser.parse(["Boltech/server/all_kml.php?entries=20&cliente=672", "cerca/cercas_kml.php?cliente=672"]);
				else
					myParser.parse(["Boltech/server/all_kml.php?entries=20&cliente=672"]);				
				
				if (geInstalado && ge.earthVisible_)
					ge.addKML_("Boltech/server/all_kml.php?entries=20&cliente=672");
				...
				if (chk.checked == true)
					myParser.parse(["Boltech/server/kml.php?entries="+ entries +"&imei=" + imei, "cerca/cercas_kml.php?cliente=672"]);
				else
					myParser.parse(["Boltech/server/kml.php?entries="+ entries +"&imei=" + imei]);

				if (geInstalado && ge.earthVisible_)
					ge.addKML_("Boltech/server/kml.php?entries="+ entries +"&imei=" + imei);
	...
	function verNoMapa(lat, lon) {
		...
		var image = '/Boltech//imagens/coordenada.png';
...
input.smallCheckbox
{
    ...
    <body onload="resizeDiv();initialize();updateMyKml(document.getElementById('nrImeiMapa').value);" onresize="resizeDiv();" onunload="">
		...
		Visualize o <a style="color:#FFFFFF" href="Boltech//mapa_todos.php?imei=ALL" target="_blank">Mapa Grande</a>, Com todos os veiculos.
	</div>
	    ...
	    <div style="height:5px;">
		    <span class="linkStyle">
			    <a href="#" onclick="aumentarLista();return false;" class="linkStyle"><img border="0" src="Boltech/imagens/aumenta_lista.gif" title="(+)" alt="(+)" /> aumentar lista </a> | 
			    <a href="#" onclick="diminuirLista();return false;" class="linkStyle">diminuir lista <img border="0" src="Boltech/imagens/diminui_lista.gif" title="(-)" alt="(-)" /> </a> |
			    <a href="Boltech/javascript:;" onclick="abreFecha('pano');" class="linkStyle">exibir e ocultar o street view</a>
